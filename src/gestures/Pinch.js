...
class Pinch extends Gesture {
  ...
  /**
   * Event hook for the move of a gesture. Determines if the two points moved closer together from each other between
  ...
  move(inputs) {
    if (inputs.length === DEFAULT_INPUTS) {
      ...
      var lastDistance = util.distanceBetweenTwoPoints(inputs[0].previous.x, inputs[1].previous.x, inputs[0].previous.y, inputs[1].previous.y);
